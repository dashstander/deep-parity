...
def hadamard(key, p, shape):
    tensor = jax.random.bernoulli(key, p, shape)
    return -1. * jnp.sign(tensor - 0.5)


def perceptron_hadamard_init(n, model_dim, key):

    key1, key2 = jax.random.split(key)
    
    model = Perceptron(n, model_dim, key1, use_bias=False)

    had_weights = hadamard(key2, 0.5, (model_dim, n)) 
    #had_weights = generate_boolean_cube(n).astype(float)
    had_parity = jnp.expand_dims(had_weights.prod(axis=1), 0)
    unembed_weights = jnp.concat([had_parity, -1. * had_parity], axis=0)
 
    model = eqx.tree_at(lambda m: m.linear.weight, model, had_weights / jnp.sqrt(n))
    model = eqx.tree_at(lambda m: m.unembed.weight, model, unembed_weights)
    return model
...
def save_checkpoint_to_gcs(bucket_name, model, opt_state, rng_key, current_step, config):
    ...
    seed = config['seed']
    checkpoint_dir = Path(f"hadamard/one-layer/model_dim={model_dim}/n={n}/seed={seed}")
